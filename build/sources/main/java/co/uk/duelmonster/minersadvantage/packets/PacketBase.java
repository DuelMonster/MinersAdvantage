...
public class PacketBase implements IMessage {
	...
	public PacketBase(NBTTagCompound tags) {
		this.tags = tags;
	}
	...
	public static class ClientHandler implements IMessageHandler<PacketBase, PacketBase> {
		@Override
		public PacketBase onMessage(PacketBase message, MessageContext context) {
			PacketID pID = PacketID.valueOf(message.tags.getInteger("ID"));
			PacketBase rtrnPacket = null;
			...
			if (pID != null) {
				...
				case Veinate:
					...
					if (isOreVein || settings.bExcavationEnabled && (KeyBindings.excavation_toggle.isKeyDown() || KeyBindings.excavation_layer_only_toggle.isKeyDown())) {
						...
						Variables.get().IsSingleLayerToggled = (!settings.bToggleMode && KeyBindings.excavation_layer_only_toggle.isKeyDown());
						
						ExcavationHandler.instance.processClientMessage(message, context);
						rtrnPacket = new PacketBase(message.tags);
					}
					break;
	...
	public static class ServerHandler implements IMessageHandler<PacketBase, PacketBase> {
		@Override
		public PacketBase onMessage(PacketBase message, MessageContext context) {
