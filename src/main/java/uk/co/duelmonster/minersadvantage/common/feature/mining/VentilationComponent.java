...
/**
 * VentilationComponent keeps this part of MinersAdvantage running without turning server ticks into confetti.
...
public final class VentilationComponent implements ComponentLifecycle {
  ...
  private VentilationCoreService.VentilationBatch lastBatch = new VentilationCoreService.VentilationBatch(0, 0,
      List.of());
  ...
  public void tick() {
    ...
    if (service.isCave(64, context.blockY())) {
      lastBatch = service.buildBatch(progress, config.width(), config.height(), config.depth(),
