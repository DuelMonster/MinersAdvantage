...
public final class SubstitutionCoreService {
  // Parity note: core tool-ranking and decision behavior is ported; deeper combat/tool simulation parity is tracked in checklist. (future-you will thank present-you).
   ...
   * It's here to make the behavior obvious, reliable, and slightly less mysterious at 2 AM.
   */
  public record SubstitutionDecision(String selectedToolId, boolean switched, boolean switchBackToPrimary,
      String mode) {
  }
   ...
   */
  private RankingMode resolveMode(boolean favourSilkTouch, boolean favourFortune, boolean combatContext,
  ...
  private double candidateScore(ToolCandidate candidate, RankingMode mode) {
    return switch (mode) {
      ...
      case MINING_SILK ->
        (candidate.silkTouchLevel() * 1000.0) + (candidate.speedScore() * 100.0) + candidate.fortuneLevel();
      case MINING_FORTUNE ->
        (candidate.fortuneLevel() * 1000.0) + (candidate.speedScore() * 100.0) + candidate.silkTouchLevel();
      case GENERAL -> (candidate.speedScore() * 100.0) + (candidate.attackScore() * 10.0) + candidate.fortuneLevel()
          + candidate.silkTouchLevel();
      case RESTORE -> 0.0;
    };
  }
   ...
   */
  private Comparator<ToolCandidate> comparatorFor(boolean favourSilkTouch, boolean favourFortune,
