...
public final class MAShapePrecomputeCache {
    ...
    public static void warmupFromConfig(SyncedClientConfig config) {
        ...
        warmupFeature(FeatureId.EXCAVATION, value.excavation().width(), value.excavation().height(), value.excavation().depth());
        warmupFeature(FeatureId.SHAFTANATION, value.shaftanation().width(), value.shaftanation().height(), value.shaftanation().depth());
        ...
        LogUtils.logDebug(
            ...
            envelopeBefore,
            envelopeAfter,
            Math.max(0, envelopeAfter - envelopeBefore),
    ...
    public static Set<BlockPos> excavationEnvelopeOffsets(
        ...
        return PRECOMPUTED_EXCAVATION_ENVELOPES.computeIfAbsent(key, MAShapePrecomputeCache::precomputeExcavationEnvelopeOffsets);
    }
