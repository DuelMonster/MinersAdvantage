...
public class LumbinationHelper {
	...
	public static boolean isLog(BlockState state) { return state.isIn(BlockTags.LOGS) || (state.getBlock() instanceof RotatedPillarBlock && state.getMaterial() == Material.WOOD); }

	public static boolean isLeaves(BlockState state) { return state.isIn(BlockTags.LEAVES) || (state.getBlock() instanceof LeavesBlock && state.getMaterial() == Material.LEAVES); }
	...
	public AxisAlignedBB identifyTree(BlockPos oPos, BlockState state) {
		...
		if (isLog(state)) {
			...
			if (leafPos != null) { // Leaves were found so treat this as a valid tree.
				...
				int iLeafRangeIncrease = MAConfig.CLIENT.lumbination.leafRange() / 2;

				rtrnBB = trunkArea
						.expand(iLeafRangeIncrease, MAConfig.CLIENT.lumbination.leafRange(), iLeafRangeIncrease)
	...
	private int getTreeRoot(BlockPos oPos, Block wood) {
		...
		if (MAConfig.CLIENT.lumbination.replantSaplings())
	...
	private AxisAlignedBB getTrunkSize(BlockPos oPos, Block block) {
		...
		for (int iPass = 0; iPass < 2; iPass++)
			for (int yLevel = iTreeRootY; yLevel < world.getHeight(); yLevel++) {
				...
				if ((checkPos.equals(oPos) || isLog(checkState)) && !trunkPositions.contains(checkPos)) // && Functions.isPosConnected(trunkPositions, checkPos))
					trunkPositions.add(checkPos);

				for (int iLoop = 1; iLoop <= MAConfig.CLIENT.lumbination.trunkRange(); iLoop++) {
							...
							for (int zOffset = -iLoop; zOffset <= iLoop; zOffset++) {
								...
								if (isLog(checkState)) {
									...
									if (iLoop > iTrunkWidth)
										...
										// MAConfig.CLIENT.lumbinationLeaves().has(Functions.getBlockName(checkBlock)))
									iAirCount++;
							}
						} else {
							for (int zOffset = -iLoop; zOffset <= iLoop; zOffset++) {
								if (zOffset == -iLoop || zOffset == iLoop) {
									...
									if (isLog(checkState) || MAConfig.CLIENT.lumbination.logs().contains(Functions.getName(checkBlock))) {
	...
	public void replantSaplings(List<Entity> dropsHistory, BlockState leafState, BlockPos leafPos) {
		if (plantationArea != null) {
			...
			// List<ItemStack> drops = Block.getDrops(leafState, (ServerWorld)world,
			// leafPos, null);
	...
	public boolean playerHasShears() { return getPlayersShears() != null; }
