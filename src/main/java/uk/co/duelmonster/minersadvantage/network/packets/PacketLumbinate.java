...
public class PacketLumbinate extends BaseBlockPacket {
	...
	public PacketLumbinate(BlockPos _pos, Direction _sideHit, int _stateID) { super(_pos, _sideHit, _stateID); }
	public PacketLumbinate(PacketBuffer buf) { super(buf); }

	@Override
	public PacketId getPacketId() { return PacketId.Lumbinate; }
	...
	public static PacketLumbinate decode(PacketBuffer buf) { return new PacketLumbinate(buf); }
	...
	public static void handle(final PacketLumbinate pkt, Supplier<Context> ctx) {
		ctx.get().enqueueWork(() -> {
			...
			player.getServer().deferTask(new Runnable() {
				@Override
				public void run() { AgentProcessor.INSTANCE.startProcessing(player, new LumbinationAgent(player, pkt)); }
			});
