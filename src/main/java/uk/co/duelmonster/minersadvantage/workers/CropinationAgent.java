		...
		for (int iQueueCount = 0; queued.size() > 0; iQueueCount++) {
			...
			if (isFullyGrown(state)) {
				...
				for (ItemStack plantable : plantables) {
					...
					if (plantable.getCount() > 0 && (MAConfig.CLIENT.cropination.harvestSeeds() || !(plantable.getItem() instanceof BlockNamedItem)))
				...
				if (!MAConfig.CLIENT.cropination.harvestSeeds() && oSeeds != null)
