...
public class ClientEventHandler {
	...
	public static void onClientTick(TickEvent.ClientTickEvent event) {
		Variables variables = Variables.get();
		if (event.phase != Phase.END || !variables.HasPlayerSpawned)
			return;

		Minecraft mc = ClientFunctions.mc;

		if (MAConfig.CLIENT.captivation.enabled() && !mc.isGamePaused()
				&& (mc.isGameFocused() || MAConfig.CLIENT.captivation.allowInGUI())) { NetworkHandler.sendToServer(new PacketCaptivate()); }
	...
	public static void onItemRightClick(PlayerInteractEvent.RightClickItem event) {
		...
		if (world.isRemote
			...
			if (state.isIn(BlockTags.DIRT_LIKE) || state.getBlock().equals(Blocks.GRASS_PATH)) { NetworkHandler.sendToServer(new PacketPathanate(pos)); }
		}
	}
