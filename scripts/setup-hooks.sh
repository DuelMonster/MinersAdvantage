...
chmod +x .githooks/pre-commit .githooks/commit-msg scripts/validate-docs.sh scripts/validate-changelog.sh scripts/validate-optimization-pass.sh scripts/validate-compile-matrix.sh 2>/dev/null || true
